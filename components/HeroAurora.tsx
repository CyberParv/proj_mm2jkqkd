...
export default function HeroAurora({
  badge = "High-Intensity Training Studio",
  headline = "Train Harder. Move Faster. Feel Unstoppable.",
  subheadline = "Join the city’s most electrifying fitness community with HIIT, strength, and recovery under one roof.",
  primaryCta = { label: "Book a Free Trial", href: "#contact" },
  secondaryCta = { label: "View Classes", href: "#classes" },
}: Partial<HeroAuroraProps>) {
  return (
    <AuroraBackground auroraColors={["#FF2E00", "#FF6A00", "#FFB347", "#FFD1C0", "#FF3B30"]}>
      <motion.div
      ...
      >
        {badge && (
          <span className="mb-2 inline-block rounded-full border bg-background/50 backdrop-blur-sm px-4 py-1.5 text-sm font-medium text-foreground uppercase">
            {badge}
          </span>
        )}
        <TextGenerateEffect
